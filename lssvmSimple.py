...
def bin_class():
    ...
    v1, v2 = np.meshgrid(np.linspace(v1_min,v1_max,50),np.linspace(v2_min, v2_max,50))
    s = np.stack([v1.reshape(-1, 1).flatten(), v2.reshape(-1, 1).flatten()], axis=1)
    #-----------------------------
    ...
    X_train, X_test, y_train, y_test = train_test_split(df, target, test_size=0.90, random_state=42)
